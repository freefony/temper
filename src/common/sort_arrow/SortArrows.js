...
const sortArrows = ({moveAction, canMoveUp = true, canMoveDown = true, position}) => {
  ...
  return <div className={`flex flex-col ${justify}`}>
    {canMoveUp && <div className='arrow-btn cursor-pointer' onClick={() => moveAction('up', position)} title={`arrow-up-${position}`}><ArrowUp /></div>}
    {canMoveDown && <div className='arrow-btn cursor-pointer' onClick={() => moveAction('down', position)} title={`arrow-down-${position}`}><ArrowDown /></div>}
  </div>
}
